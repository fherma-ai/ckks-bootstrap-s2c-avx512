# CKKS bootstrapping, S2C-first — the AVX-512 submission

The harness as `fherma implementation init ckks-bootstrapping/s2c@1.0.0` writes
it, with two lines changed: `Cargo.toml` selects the `avx512` backend
(AVX-512F, single-threaded) and `config.jsonc` asks for 1 thread.
`src/run.rs` is the reference's, untouched: this row competes on backend and
threads, not on the algorithm.

Poulpy publishes 10.2 s for this backend on a Ryzen 9 9950X. The number here
